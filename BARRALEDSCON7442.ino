...
void setup(){ //Configuración
 pinMode(9,OUTPUT);pinMode(8,OUTPUT);//Pin 9, 8 OUT
 pinMode(7,OUTPUT);pinMode(6,OUTPUT);//Pin 7, 6 OUT
...
void loop(){ //Super lazo infinito
 pote=analogRead(A0);//Toma canal análogo
 pote=map(pote,0,1023,0,9);//covierte rangos
